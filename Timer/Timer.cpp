...
{
    static int x,y, m = 1, s = 0,IDtimer,vt,xuong=0;
    static TCHAR kq[30];   
    HDC hdc;
    switch (message)
    {
    ...
    case WM_TIMER:
        ...
        case 1:
            ...
            {
                KillTimer(hWnd, 1);
                MessageBox(hWnd, TEXT("Hết giờ"), TEXT("Thông Báo"), MB_OK);
            }
            wsprintfW(kq, L"Time left: %d:%d", m, s);
            hdc = GetDC(hWnd);
            TextOut(hdc, x - 150, y - 20, kq, 30);
            ReleaseDC(hWnd, hdc);
            break;
        case 2:
            if (xuong == 0 )
            ...
            hdc = GetDC(hWnd);
            TextOut(hdc, 50, vt, TEXT("hello"), 5);
            ReleaseDC(hWnd, hdc);
            break;
        default:
            break;
