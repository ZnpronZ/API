...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    static POINT pointTG[3], pointHT[4];
    static HPEN hPen;
    ...
    case WM_LBUTTONUP:
        ...
        if (hinh == ID_HCN)
            Rectangle(myHDC, xLeft, yTop, xRight, yBottom);
        if(hinh==ID_ESCLIP)
            Ellipse(myHDC, xLeft, yTop, xRight, yBottom);
        
        if (hinh == ID_TGC)
        {
            pointTG[0].x = xLeft + (xRight - xLeft)/2;
            pointTG[0].y = yTop;
            pointTG[1].x = xRight;
            pointTG[1].y = yBottom;
            pointTG[2].x = xLeft;
            pointTG[1].y = yBottom;
            Polygon(myHDC, pointTG, 3);
        }
